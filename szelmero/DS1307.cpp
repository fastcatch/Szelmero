...
int DS1307::get(int c, boolean refresh)  // aquire individual RTC item from buffer, return as int, refresh buffer if required
{
  ...
  case DS1307_YR:
      ...
      v=rtc_bcd[DS1307_YR]/16 * 10 + rtc_bcd[DS1307_YR] % 16;
...
void DS1307::start(void)
{
  ...
  read();
  rtc_bcd[DS1307_SEC] = rtc_bcd[DS1307_SEC] & (uint8_t)(~DS1307_CLOCKHALT);
  save();

  Wire.beginTransmission(DS1307_CTRL_ID);
  Wire.send(DS1307_CTRL_REGISTER); // set register pointer  
  Wire.send(0x10); // set register: SQW out enabled
  Wire.endTransmission();
}
